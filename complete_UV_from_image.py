...
def compute_uv_texture(args):
    ...
    uv_v_bary_xy = cameras.transform_points_screen(uv_v_bary)[:, :, :2] # n_view x uv_size x 2
    ...
    uv_vc = torch.zeros_like(uv_v_bary)[None].repeat(len(imgs), 1, 1)
    ...
    for i in range(len(imgs)):
        ...
        uv_vc[i] = x1y1 * w11 + x1y2 * w12 + x2y1 * w21 + x2y2 * w22
        uv_vc_i = uv_vc[i].detach().cpu().numpy().reshape(args.uv_size, args.uv_size, 3)
        mask_uv_i = uv_vc_i[:, :, 0] > 0

        cv2.imwrite('results/image_%03d.png' % i, (uv_vc_i))
